...
<% 
	...
	//세션에 검색 내용이 없을 때 전체 내용 호출
		...
		sql.append(" SELECT COUNT(*) FROM " + TABLE_WRITE + "WHERE Write_kind='NEWS'");
		...
		String sqlList = "SELECT * FROM "+ TABLE_WRITE +" WHERE Write_kind='NEWS' order by Write_date desc";

		rs = stmt.executeQuery( sqlList );
	} else {
		//세션에 검색이 있을 경우
		sql = new StringBuffer();
		sql.append(" SELECT COUNT(*) FROM " + TABLE_WRITE + "WHERE Write_kind='NEWS' AND Write_title like '%" + srch + "%'" );
		...
		String sqlList = "SELECT * FROM "+ TABLE_WRITE +" WHERE Write_kind='NEWS' AND Write_title like '%" + srch + "%' order by Write_date desc";
...
<html>
 ...
 <body>
	...
	<!-- 본문 -->
		...
		<img src="./images/news.png" width="250px" alt="미소짓는 뉴스"/>
